...
### 2b. Confirm course visibility (only when CREATING a new course)

If this is the very first lesson for a brand-new course, decide and confirm with the user whether the course is **public** or **private**:

- **Public** (default) — appears in the catalog at `/courses/` for every visitor. Set `private: false` (or omit).
- **Private** — hidden from the public catalog. Only signed-in users with a row in the `course_access` table for this slug see the card. Set `private: true` in `meta.yaml`.

Either way, *file* access is independently enforced by storage RLS — `private` only controls discoverability.

This is easy to forget: ask explicitly. For private courses also remind the user to add the intended learners to the `course_access` table (Supabase dashboard → Table Editor → `public.course_access` → insert `(email, course_slug)` rows).
...
### 6. Publish — the three-way split

Uploading is **not the end**. The pipeline has three independent persistence layers; ending after `upload-course` leaves the catalog stale on the live site. After the upload succeeds:

1. **Commit the regenerated catalog `.md`** — `upload-course` rewrites `content/courses/<slug>.md` from `meta.yaml`. This file is tracked in git; without a commit and push it will not appear on the deployed Hugo site. Always:
   ```bash
   git add content/courses/<slug>.md
   git commit -m "Update <slug> course catalog"
   git push
   ```
2. **Grant access (private courses only)** — if `meta.yaml` has `private: true`, add allowlist rows:
   ```bash
   npm run grant-access -- <slug> alice@example.com bob@example.com
   ```
   The recipient must subsequently sign in at jurispuce.com with the matching email; granting access creates the row, not the auth user.

The three layers (Supabase storage / git-tracked catalog / `course_access` table) are independent. Skipping any of them produces a partial deploy. See `MANAGING_COURSES.md` → "Publishing changes — the three-way split" for the full table.
...
- **Don't** stop at `upload-course`. Uncommitted catalog `.md` files don't deploy. Always commit and push after upload. For private courses, also run `npm run grant-access` for the intended learners.
...
- **Don't** assume a course is public. Confirm visibility with the user when creating a new course (`private: true|false` in `meta.yaml`).
...
- `MANAGING_COURSES.md` — operational doc for the course pipeline (canonical source for ops; this skill is the authoring layer)
- `scripts/upload-course.js` — uploader (top-of-file comment block describes both meta.yaml schemas)
- `scripts/grant-course-access.js` — `npm run grant-access -- <slug> <email> [<email> ...]` for private-course allowlists
- `scripts/send-course-email.js` — `npm run send-email` for welcome / update emails to enrolled users
